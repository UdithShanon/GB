...
public class FundingPosts {
	...
	public String createPost(String title,String content) {
		...
		try {
			...
			String  query= "insert into funding_bodies(title,content,postDate,postTime"
					+ ")"
							+" values(?,?,?,?) ";
	...
	public String updatePost(String ID,String title,String content) 
	 ...
	 LocalDate date= LocalDate.now();
	 LocalTime time= LocalTime.now();
	 String query = "UPDATE Funding_Bodies_Post SET title=?,content=?,postDate=?,postTime=? WHERE id=?"; 
